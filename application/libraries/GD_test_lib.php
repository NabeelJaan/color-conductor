...
class Color_image_lib {
	...
	public function createImage($formula)
	{
		...
		$directory_chunk_png = "assets/images/chunk/";
		$chunk_array = scandir($directory_chunk_png);
			...
			for ($i = 0; $i < count($info) - 1; $i++)
			{
				...
				else
				{
					...
					if ($info[$i]["flecks_size"] === "Coarse" && ($info[$i]["r"] != 157 && $info[$i]["g"] != 107 && $info[$i]["b"] != 61))
					{
						...
						$numOfFlecks = ($coverArea / $fleckArea) - 55;
						imagecolorset($source, 0, $info[$i]["r"], $info[$i]["g"], $info[$i]["b"]);
					}
					else if($info[$i]["r"] == 157 && $info[$i]["g"] == 107 && $info[$i]["b"] == 61){
						$x = rand(2, count($fine_array) - 1);
					...
					{
						if($info[$i]["r"] != 157 && $info[$i]["g"] != 107 && $info[$i]["b"] != 61){
							$x = rand(2, count($fine_array) - 1);
					...
					for ($j = 0; $j < 3; $j++)
						{
							for ($k = 0; $k < 2; $k++)
							{
								...
								//if true then get the random image to be Chunks from Chunks array
								$x = rand(2, count($chunk_array)-1);
								$directory_chunk_png.= $chunk_array[$x];
								$white_fleck = $directory_chunk_png;
								...
								$numOfFlecks = ($coverArea / $fleckArea) - ($info[$i]['percent']*7);
								//set color for given values
								imagecolorset($source, 0, $info[$i]["r"], $info[$i]["g"], $info[$i]["b"]);
