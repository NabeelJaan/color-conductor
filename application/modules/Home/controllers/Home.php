...
class Home extends MX_Controller {	
	...
	function create_color_session(){
		...
		foreach($colors as $color){
			...
			$session_colors[$color[4]]['jumbo']=$color[4];
	...
	function create_new_color_session(){
		...
		foreach($colors as $color){
			...
			if(!array_key_exists($color[4],$mix_it_array)){
				...
				$new_array[$color[4]]['jumbo']=$color[4];
...

	{
	    ...
	    $config['smtp_user']    = 'summitintlflooringwebsite@gmail.com'; 
	    $config['smtp_pass']    = 'CIPRCOM#01'; 
	    ...
	    $config['protocol'] = 'mail';
		    ...
		    $config['smtp_user']    = 'summitmedia0800@gmail.com';    //Important
		    $config['smtp_pass']    = 'ykvzauahcednjaka';  //Important
		    $config['charset']    = 'utf-8';
		    $config['newline']    = "\r\n";
		...
		/*$this->email->cc(array('dnumark@summit-flooring.com,MHawk@summit-flooring.com,mbecker@summit-flooring.com,orders@summit-flooring.com'));	*/
		$this->email->subject('Summit Flooring - The Color Conductor - Your Favorite Swatch');

		$email_facebook_icon = '<img src="https://summit-flooring.com/color-conductor/home_assets/images/facebook.png">';

		$email_facebook = '<a href="https://www.facebook.com/pages/summitflooring/682542678475641" target="_blank">' . $email_facebook_icon . '</a>';

		$email_twitter_icon = '<img src="https://summit-flooring.com/color-conductor/home_assets/images/twitter.png">';
...

	{
       ...
       // print_r($email_info);die;
		...
		$this->email->to($email_user);
		/*$this->email->cc(array('dnumark@summit-flooring.com,MHawk@summit-flooring.com,mbecker@summit-flooring.com,orders@summit-flooring.com'));*/
		$this->email->subject('Color Conductor - Your Favorite Swatch');
...
die;
	...
	*/	
	...
	/*	$gallery_image_section = '<div class="col-md-12 " style="height: 555px; width: 100%;">					
		...
		$this->email->from('info@summit-flooring.com', 'Summit Flooring');
		$this->email->to($email_user);	
		$this->email->subject('Summit Flooring - The Color Conductor - Swatch With Gallery');
		/*$this->email->cc(array('dnumark@summit-flooring.com,MHawk@summit-flooring.com,mbecker@summit-flooring.com,orders@summit-flooring.com'));*/
  ...
  //  header("Content-Type: image/png");
...

	{
		...
		/*	$logo_image_section = '<div style="width:50%; height:50%;"><div style="width:100%;height:100%;background-image:url('.$email_image_src.');background-size:25% 25%;z-index: 100;"><div style="width:100%;height:100%;background-size:25% 25%;position:relative;background-image:url('.$email_image_src.');text-align: center;opacity: 0.6;z-index: -1;"><img src="'.$logo_image.'" style="width:30%;margin:35% auto;z-index:999;"></div></div></div>';*/
			...
			$this->email->subject('Summit Flooring - The Color Conductor - Swatch With Logo');
	...
	function add_reg_user(){
		...
		if($flag!=false){
			...
			$this->email->subject('Color Conductor - Registration Details');
...

	{
		...
		if($flag!=false){
			...
			$this->email->subject('Color Conductor - Password Recover');
...

	{
		...
		if($flag!=false){
			...
			$this->email->initialize($config);
			$this->email->from('sales@summitflooring.com', 'Color Conductor');
			...
			/*$this->email->cc(array('dnumark@summit-flooring.com,MHawk@summit-flooring.com,mbecker@summit-flooring.com,orders@summit-flooring.com'));*/
			$this->email->subject('Color-Conductor - Product Request Sample Form');
...

	{
		...
		for($i=0;$i<count($info)-1;$i++){
			...
			if($info[$i]['flecks_size']=='undefined'){
				$flecksSize="Jumbo";
			}else{
				$flecksSize=$info[$i]['flecks_size'];
			}
			...
			if($single_color != false){
				...
				$formula_list .= '<li class="col-md-12 col-sm-12 col-xs-12">
								...
								<div class="col-md-12 col-xs-12 col-sm-12">
									...
									<h5><b>'.$single_color['color_title'].'</b> - '.$flecksSize.' - '.$info[$i]['percent'].'%</h5>
	...
	public function samplereq($id){
		...
		$swatchName=str_replace('_-','_Jumbo-',$swatch['org_image_name']);

		//$org_image_name = explode(".",$swatch['org_image_name']);
		$org_image_name = explode(".",$swatchName);
